...
class Estimator {
  ...
  constructor(input: { height: number; width: number }) {
    ...
    this.focalLength = focalLength;
    this.center = [frameWidth * 0.5, frameHeight * 0.5];
  }

  public estimate(landmarks: number[][]) {
    const screenLandmarks = this.#projectToScreen(landmarks);

    let intermediateLandmarks = this.#cloneLandmarks(screenLandmarks);
    this.#changeHandedness(intermediateLandmarks);

    const depthOffset = screenLandmarks.reduce((a, l) => a + l[2], 0) / screenLandmarks.length;

    const firstIterationScale = this.#estimateScale(intermediateLandmarks);

    intermediateLandmarks = this.#cloneLandmarks(screenLandmarks);

    this.#moveAndRescaleZ(depthOffset, firstIterationScale, intermediateLandmarks);
    this.#unprojectScreen(intermediateLandmarks);
    this.#changeHandedness(intermediateLandmarks);

    const secondIterationScale = this.#estimateScale(intermediateLandmarks);
    const metricLandmarks = this.#cloneLandmarks(screenLandmarks);
    const totalScale = firstIterationScale * secondIterationScale;

    this.#moveAndRescaleZ(depthOffset, totalScale, metricLandmarks);
    this.#unprojectScreen(metricLandmarks);
    this.#changeHandedness(metricLandmarks);

    const poseTransformMat = this.#solveWeightedOrthogonal(
  ...
  #projectToScreen(landmarks: number[][]) {
    const ret: number[][] = [];
  ...
  #cloneLandmarks(landmarks: number[][]) {
    const ret: number[][] = [];
  ...
  #changeHandedness(landmarks: number[][]) {
    for (let i = 0; i < landmarks.length; i++) {
      landmarks[i][2] *= -1;
    }
  }

  #moveAndRescaleZ(depthOffset: number, scale: number, landmarks: number[][]) {
    for (let k = 0; k < landmarks.length; k++) {
      landmarks[k][2] = (landmarks[k][2] - depthOffset + this.near) / scale;
    }
  }

  #unprojectScreen(landmarks: number[][]) {
    for (let k = 0; k < landmarks.length; k++) {
  ...
  #estimateScale(landmarks: number[][]) {
    const transformMat = this.#solveWeightedOrthogonal(
      canonicalMetricLandmarks,
      landmarks,
      sqrtWeights
    );
  ...
  #solveWeightedOrthogonal(sources: number[][], targets: number[][], sqrtWeights: number[]) {
    ...
    const rotation = this.#computeOptimalRotation(designMatrix);

    const scale = this.#computeOptimalScale(
  ...
  #computeOptimalRotation(designMatrix: number[][]) {
